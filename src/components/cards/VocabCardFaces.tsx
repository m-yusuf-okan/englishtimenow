  ...
  return (
    ...
    >
      <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center">
        {reversed ? (
          <p className="text-3xl sm:text-4xl leading-tight font-bold text-balance">
            {card.translation}
          </p>
        ) : (
          <>
            <p
              lang="en"
              className="text-4xl sm:text-5xl leading-tight font-bold tracking-tight text-balance"
            >
              {card.term}
            </p>
            <p className={`font-mono text-sm sm:text-base ${tokens.muted}`}>{card.ipa}</p>
          </>
        )}
      </div>
    </CardShell>
      ...
      <div className="flex flex-1 flex-col gap-5 overflow-y-auto">
        {reversed ? (
          <div>
            ...
            <p className={`mt-1 font-mono text-xs sm:text-sm ${tokens.muted}`}>{card.ipa}</p>
          </div>
        ) : (
          <p className="text-2xl sm:text-3xl leading-tight font-bold text-balance">
            {card.translation}
          </p>
        )}

        <div className="space-y-1.5">
          <p lang="en" className="text-base sm:text-lg leading-relaxed font-medium">
            {card.example.en}
          </p>
          {/* PRD 2.C: örnek cümlenin Türkçesi parantez içinde, hemen alt satırda. */}
          <p className={`text-xs sm:text-sm leading-relaxed ${tokens.muted}`}>({card.example.tr})</p>
        </div>
